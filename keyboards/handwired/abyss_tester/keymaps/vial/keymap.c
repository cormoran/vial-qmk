...
#ifdef VIAL_ENABLE
static const char *const default_dynamic_macros[] = {
    "abyss-vial-00",
    "abyss-vial-01",
    "abyss-vial-02",
    "abyss-vial-03"
...
static const vial_tap_dance_entry_t default_dynamic_tap_dances[] = {
    [TD_ESC_CAPS] = {KC_ESC, KC_NO, KC_NO, KC_NO, TAPPING_TERM},
    [TD_SPC_ENT]  = {KC_SPC, KC_ENT, KC_NO, KC_NO, TAPPING_TERM},
    [TD_CTL_CAPS] = {KC_NO, KC_LCTL, KC_CAPS, KC_NO, TAPPING_TERM}
};

static const vial_combo_entry_t default_dynamic_combos[] = {
    {.input = {KC_Q, KC_W, COMBO_END, COMBO_END}, .output = KC_TAB},
    {.input = {KC_J, KC_K, KC_L, COMBO_END}, .output = KC_ESC},
    {.input = {KC_Z, KC_X, KC_C, KC_V}, .output = ABYS_1},
    {.input = {KC_C, KC_V, COMBO_END, COMBO_END}, .output = ABYS_2}
};

static const vial_key_override_entry_t default_dynamic_key_overrides[] = {
    {
        .trigger           = KC_BSPC,
        .replacement       = KC_DEL,
        .layers            = 0xFFFF,
        ...
        .options           = vial_ko_enabled | vial_ko_option_activation_negative_mod_up | vial_ko_option_activation_required_mod_down | vial_ko_option_activation_trigger_down
    },
    {
        .trigger           = KC_ESC,
        .replacement       = S(KC_GRV),
        .layers            = 0xFFFF,
        .trigger_mods      = MOD_MASK_SHIFT,
        .negative_mod_mask = 0,
        .suppressed_mods   = MOD_MASK_SHIFT,
...
static const vial_alt_repeat_key_entry_t default_dynamic_alt_repeat_keys[] = {
    {.keycode = KC_LEFT, .alt_keycode = KC_RGHT, .allowed_mods = 0, .options = vial_arep_enabled | vial_arep_option_bidirectional},
    {.keycode = KC_UP, .alt_keycode = KC_DOWN, .allowed_mods = 0, .options = vial_arep_enabled | vial_arep_option_bidirectional}
};

static bool write_default_dynamic_macro(uint16_t *offset, uint16_t macro_buffer_size, const char *macro) {
    uint16_t length = strlen(macro) + 1;

    if (*offset + length > macro_buffer_size) {
        return false;
    }

    while (length > 0) {
        uint8_t chunk[16];
        uint16_t chunk_size = length > sizeof(chunk) ? sizeof(chunk) : length;

        memcpy(chunk, macro, chunk_size);
        dynamic_keymap_macro_set_buffer(*offset, chunk_size, chunk);
...
void dynamic_keymap_macro_reset_user(void) {
    ...
    dynamic_keymap_macro_set_buffer(macro_buffer_size - 1, 1, &valid_marker);

    uint16_t offset = 0;
    for (uint8_t i = 0; i < sizeof(default_dynamic_macros) / sizeof(default_dynamic_macros[0]); i++) {
        if (!write_default_dynamic_macro(&offset, macro_buffer_size, default_dynamic_macros[i])) {
            break;
        }
    }
...
void dynamic_keymap_tap_dance_reset_user(void) {
    for (uint8_t i = 0; i < sizeof(default_dynamic_tap_dances) / sizeof(default_dynamic_tap_dances[0]); i++) {
        dynamic_keymap_set_tap_dance(i, &default_dynamic_tap_dances[i]);
    }
}

void dynamic_keymap_combo_reset_user(void) {
    for (uint8_t i = 0; i < sizeof(default_dynamic_combos) / sizeof(default_dynamic_combos[0]); i++) {
        dynamic_keymap_set_combo(i, &default_dynamic_combos[i]);
    }
}

void dynamic_keymap_key_override_reset_user(void) {
    for (uint8_t i = 0; i < sizeof(default_dynamic_key_overrides) / sizeof(default_dynamic_key_overrides[0]); i++) {
        dynamic_keymap_set_key_override(i, &default_dynamic_key_overrides[i]);
    }
}

void dynamic_keymap_alt_repeat_key_reset_user(void) {
    for (uint8_t i = 0; i < sizeof(default_dynamic_alt_repeat_keys) / sizeof(default_dynamic_alt_repeat_keys[0]); i++) {
        dynamic_keymap_set_alt_repeat_key(i, &default_dynamic_alt_repeat_keys[i]);
    }
}
#endif
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_CUSTOM] = LAYOUT(
        ...
        TD(TD_CTL_CAPS), KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_NO,   DM_REC2, DM_PLY2,
